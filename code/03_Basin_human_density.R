...
# ========================== Basins (once) =====================================
# Read basin polygons once, normalize around the dateline, and fix topology
...
# build an equal-area grid for ONE basin, clipped to basin extent
build_basin_grid <- function(basin_sf, grid_km = GRID_KM) {
  basin_eq <- sf::st_transform(basin_sf, EQUAL_AREA_CRS)
  ...
  sf::st_make_grid(
    ...
    sf::st_intersection(basin_eq) %>%
    sf::st_as_sf() %>%
    dplyr::mutate(grid_id = dplyr::row_number(), .before = 1)
}

# Summarize population density for ONE species in ONE basin across occupied cells,
...
#   grid_eq     : sf grid (equal-area cells) for this basin
...
summarize_population_for_species <- function(ras_files, grid_eq, keep_i, sp) {
  ...
  if (length(keep_i) == 0L) {
    # species has no occupied cells in this basin
...
process_basin <- function(basin_id_value) {
  ...
  if (!is.data.frame(spp_raw) || nrow(spp_raw) == 0L) {
    cli::cli_alert_warning("Species table empty/invalid for basin {basin_id_value} - skipping")
    return(NULL)
  }
  
  if (!"valid_name" %in% names(spp_raw)) {
    cli::cli_alert_warning("'valid_name' missing in {spp_path} - skipping basin {basin_id_value}")
    return(NULL)
  }
  
  # keep only basin_id + valid_name, one row per species
  spp_tbl <- spp_raw %>%
    dplyr::select(basin_id, valid_name) %>%
    dplyr::distinct(valid_name, .keep_all = TRUE)
  
  # ---- basin geometry ----
  basin_sf <- inland %>% dplyr::filter(.data$basin_id %in% basin_id_value)
  if (nrow(basin_sf) == 0L) {
    cli::cli_alert_warning("No geometry for basin {basin_id_value} - skipping")
    return(NULL)
  }
  ...
  grid_eq <- tryCatch(
    {
      build_basin_grid(basin_sf, grid_km = GRID_KM)
    },
    error = function(e) {
      cli::cli_alert_warning(
        "build_basin_grid() failed for {.bold {basin_id_value}}: {conditionMessage(e)}"
      )
      NULL
    }
  )
  
  # helper: NA row for a species when we can't compute densities
  ...
  # If grid is NULL or empty, we can't do intersections or extraction.
  # We still output NA rows for every species.
  if (is.null(grid_eq) || nrow(grid_eq) == 0L) {
    cli::cli_alert_warning(
      "No usable grid for {.bold {basin_id_value}} (NULL or 0 rows). Writing NA densities."
    )
    ...
    cli::cli_alert_success("Saved basin-level population table: {out_csv}")
    return(invisible(out_csv))
  }
  ...
  for (sp in unique(spp_tbl$valid_name)) {
    cli::cli_alert_info(
      "Processing species {.bold {sp}} in basin {.bold {basin_id_value}}"
    )
    
    # read occurrence for this species
    occ_path <- file.path(
      DIR_OCC,
    ...
    occ_sf <- if (file.exists(occ_path)) {
      ...
      as_occ_sf(occ_raw)  # must return sf POINTS with "year"
    } else {
      NULL
    }
    
    # identify occupied grid cells in equal-area space
    if (is.null(occ_sf) || nrow(occ_sf) == 0L) {
      cli::cli_alert_warning(
        "No valid occurrences for {.bold {sp}} in basin {.bold {basin_id_value}}"
      )
      keep_i <- integer(0)
    } else {
    ...
    # summarize population density (current period)
    dens_current <- tryCatch(
      {
        summarize_population_for_species(
      ...
      error = function(e) {
        cli::cli_alert_warning(
          "summarize_population_for_species(current) failed for {.bold {sp}} in {.bold {basin_id_value}}: {conditionMessage(e)}"
        )
        NULL
      }
    )
    
    # summarize population density (past period)
    dens_past <- tryCatch(
      {
        summarize_population_for_species(
      ...
      error = function(e) {
        cli::cli_alert_warning(
          "summarize_population_for_species(past) failed for {.bold {sp}} in {.bold {basin_id_value}}: {conditionMessage(e)}"
        )
        NULL
      }
    )
    ...
    # build final rows for this species
    if (is.null(dens_all) || nrow(dens_all) == 0L) {
      merged_sp <- make_na_row_for_species(sp)
    } else {
      # dens_all is expected to have: valid_name, year, population_density
      merged_sp <- dens_all %>%
        dplyr::mutate(
          basin_id = basin_id_value
        ) %>%
    ...
    cli::cli_alert_info(
      "Finished {.bold {sp}} in basin {.bold {basin_id_value}} ({nrow(merged_sp)} rows)"
    )
  }
  
  # ---- combine all species, clean "year", enforce columns ----
  ...
  # final safety: if somehow results was empty
  if (nrow(out_tbl) == 0L) {
...
for (i in basin_ids) { #seq_along(basin_ids
  
  
  # guard the loop so one bad basin doesn't kill the run
  try(process_basin(i), silent = TRUE)
