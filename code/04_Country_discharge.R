...
DIR_OUT        <- "input/data_prep/country_discharge"
DIR_RAST       <- "input/processed/discharge_annual_mean_ensemble"
...
ras_files <- list.files(
  DIR_RAST,
  pattern = "^discharge_annual_mean_ensemble.*\\.(tif|tiff)$",
  full.names = TRUE
)
    ...
    build_na_rows <- function() {
      purrr::map(ras_files, function(fp) {
        ...
        tibble::tibble(
          ...
          discharge  = rep(NA_real_, terra::nlyr(r))
        )
      }) %>% dplyr::bind_rows()
    }
        ...
        if (length(keep_i) == 0L) {
          ...
          sp_rows <- purrr::map(ras_files, function(fp) {
            ...
            tibble::tibble(
              valid_name = sp,
              year       = layer_years(r, filepath = fp),
              discharge  = as.numeric(lyr_means)
            )
          }) %>% dplyr::bind_rows()
        }
      }
