...
# - names(preds): transformed predictor variables (all "z_*" columns)
...
for (config in model_configs) {
  ...
  raw_path   <- file.path(output_dir, sprintf("country_darwinian_%s_raw.rds",  model_name))
  final_path <- file.path(output_dir, sprintf("country_darwinian_%s.rds",      model_name))
...
if (length(fits) > 0L) {
  all_path <- file.path(output_dir, "country_darwinian_all.rds")
  ...
  warning("[WARNING] No models were successfully fitted; country_darwinian_all.rds not written.")
}
...
if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)
output_path <- file.path(output_dir, "country_darwinian_model_comparison")
