...
base_df <- read_parquet("input/data_prep/wallacean_basin_single.parquet")
# base_df <- read_parquet("input/data_prep/wallacean_basin_multiple.rds")
...
cutoff_year <- 2024  # use the same cutoff as in the wallacean model for consistency
...
data_surv$watershed_area <- log10(data_surv$watershed_area+1)
data_surv$range_size <- log10(data_surv$range_size+1)
data_surv$elevation <- log10(data_surv$elevation+30)
...
#saveRDS(stan_survdata,"stan_survdata_basin_wallacean.rds")
# ------------------------------------------------------------
...
for (config in model_configs) {
  ...
  raw_path   <- file.path(output_dir, sprintf("basin_wallacean_%s_raw.rds",  model_name))
  final_path <- file.path(output_dir, sprintf("basin_wallacean_%s.rds",      model_name))
...
if (length(fits) > 0L) {
  all_path <- file.path(output_dir, "basin_wallacean_all.rds")
  ...
  warning("[WARNING] No models were successfully fitted; basin_wallacean_all.rds not written.")
}
...
if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)
output_path <- file.path(output_dir, "basin_wallacean_model_comparison")
...
saveRDS(fit_gomp, file.path("output/model", "basin_wallacean_gompertz.rds"))
