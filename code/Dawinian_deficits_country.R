...
#   - "continent": Calculate for continent (e.g., Africa)
#   - "iso3": Calculate for drainage basins (via unique country codes)
...
config <- list(
  # File paths
  data_path = "input/processed/country_sp.rds",
  biogeo_path = "input/raw/country_list.csv",
  tree_dir = "input/processed/tree_fish/",
  tree_prefix = "tree_fish_18k_n100_",
  tree_suffix = ".rds",
  output_csv = "output/tables/pd_deficits_all_country.csv",  # Default for no grouping
  ...
  # Flexible grouping: 
  # - Use "continent" or "iso3" for grouped analysis
  # - Use "none" to analyze all species together (no grouping)
  grouping_col = "continent"  # Current setting: no grouping (all species in one group)
)

# Auto-update output filename based on grouping type
if (config$grouping_col == "none") {
  config$output_csv <- "output/tables/pd_deficits_all_country.csv"  # Explicit name for global analysis
} else {
...
preprocess_group_data <- function(df, group_col) {
  ...
  # Case 2: Grouped analysis (continent or iso3)
  df %>%
...
results_summary <- final_results %>%
  ...
  # - Use the specified column (e.g., "iso3", "continent") for grouped analysis
  ...
  group_by(
    !!sym(ifelse(
      ...
      config$grouping_col             # Grouped: Use specified column (e.g., "iso3")
    ))
  ) %>%
  summarise(
...
# results_summary <- results_summary %>% left_join(biogeo_lookup, by = "iso3")
# Show top 10 groups (for quick inspection)
cat(sprintf("\nTop 10 %s by Mean PD Deficit:\n", config$grouping_col))
print(head(results_summary, 10))
write.csv(results_summary,sprintf("output/tables/pd_deficits_%s_summary.csv", config$grouping_col),row.names = F)
