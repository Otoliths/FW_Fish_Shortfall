...
base_df <- read_parquet("input/data_prep/wallacean_country_single.parquet")
...
country_discharge_avg <- read_parquet("input/data_prep/country_discharge_avg.parquet")
country_watertemp_avg <- read_parquet("input/data_prep/country_watertemp_avg.parquet")
country_rarity_avg <- read_parquet("input/data_prep/country_rarity_avg.parquet")
country_population_density_avg <- read_parquet("input/data_prep/country_population_density_avg.parquet")
country_elevation_avg <- read_parquet("input/data_prep/country_elevation_avg.parquet")
country_latitude_avg <- read_parquet("input/data_prep/country_latitude_avg.parquet")

data_filled <- data %>%
  # ---- 1) discharge ----
left_join(
  country_discharge_avg %>%
    select(iso3, discharge_avg = discharge) %>%
    filter(!is.na(discharge_avg)),
  by = c("iso3")
) %>%
  mutate(
    discharge = if_else(
      is.na(discharge) & is.na(year_geolocation),
      discharge_avg,
      discharge
    )
  ) %>%
  ...
  # ---- 2) watertemp ----
left_join(
  country_watertemp_avg %>%
    select(iso3, watertemp_avg = watertemp) %>%
    filter(!is.na(watertemp_avg)),
  by = c("iso3")
) %>%
  mutate(
    watertemp = if_else(
      is.na(watertemp) & is.na(year_geolocation),
      watertemp_avg,
      watertemp
    )
  ) %>%
...
left_join(
  country_rarity_avg %>%
    select(iso3, valid_name,range_rarity_avg = range_rarity_mean),
  by = c("iso3","valid_name")
) %>%
  mutate(
    range_rarity = if_else(
      is.na(range_rarity) & is.na(year_geolocation),
      range_rarity_avg,
      range_rarity
    )
  ) %>%
...
left_join(
  country_population_density_avg %>%
    select(iso3, population_density_avg = population_density) %>%
    filter(!is.na(population_density_avg)),
  by = c("iso3")
) %>%
  mutate(
    population_density = if_else(
      is.na(population_density) & is.na(year_geolocation),
  ...
  # ---- 5) elevation ----
left_join(
  country_elevation_avg %>%
    select(iso3, elevation_avg = elevation) %>%
    filter(!is.na(elevation_avg)),
  by = c("iso3")
) %>%
  mutate(
    elevation = if_else(
      is.na(elevation) & is.na(year_geolocation),
      elevation_avg,
      elevation
    )
  ) %>%
  ...
  # ---- 6) latitude ----
left_join(
  country_latitude_avg %>%
    select(iso3, latitude_avg = latitude) %>%
    filter(!is.na(latitude_avg)),
  by = c("iso3")
) %>%
  mutate(
    latitude = if_else(
      is.na(latitude) & is.na(year_geolocation),
      latitude_avg,
      latitude
    )
  ) %>%
...
data_filled <- data_filled %>% left_join(
  ...
  mutate(
    taxonomic_activity = if_else(
      is.na(taxonomic_activity) & is.na(year_geolocation),
...
rm(country_discharge_avg,country_watertemp_avg,country_rarity_avg,country_elevation_avg,
   country_population_density_avg,cas,data,base_df,country_latitude_avg)
...
# We now treat the availability of a *georeferenced* occurrence record
# as a time-to-event process at the species level.
#
# - year_description: year when the species was formally described
# - year_geolocation: earliest year when a georeferenced collection
#                     (with coordinates) became available for that species
#
# For species with at least one georeferenced record (year_geolocation not NA),
#   time  = year_geolocation - year_description
#   event = 1  (the "geolocation event" has occurred)
#
# For species without any georeferenced record (year_geolocation is NA),
#   we treat them as right-censored at a fixed cutoff year (e.g. 2024):
#   time  = cutoff_year - year_description
#   event = 0  (no georeferenced record observed up to cutoff_year)
#
# This definition allows us to fit a Wallacean survival model with both
# observed events (species with at least one georeferenced record) and
# right-censored species (no georeferenced record yet), thereby
# quantifying the Wallacean shortfall in a coherent way.

cutoff_year <- 2024  # use the same cutoff as in the Darwin model for consistency

data_surv <- data_filled %>%
  # Keep species with a valid description year
  filter(!is.na(year_description)) %>%
  mutate(
    # Indicator for whether the species has at least one georeferenced record
    has_geo = !is.na(year_geolocation),
    
    # Survival time in years:
    # - if has_geo: lag between description and first georeferenced record
    # - if no geo: time from description to cutoff_year (right-censored)
    time = if_else(
      has_geo,
      year_geolocation - year_description,
      cutoff_year     - year_description
    ),
    
    # Event indicator:
    # 1 = georeferenced record observed, 0 = right-censored at cutoff_year
    event = if_else(has_geo, 1L, 0L)
  ) %>%
...
# - iso3: country identifier (nested within continent)
# - continent: one of the six continent
...
# - time: geolocation lag time (years between species description and its first georeferenced record)
# - event: 1 = geolocation, 0 = not yet georeferenced by cutoff_year
...
#saveRDS(stan_survdata,"stan_survdata_country_wallacean_single.rds")
# ------------------------------------------------------------
...
preds <- c(
  ...
  "preserved_specimen",
  "sequencing_effort",
  #"sampling_effort", #removed sampling effort avoid circularity in Wallacean modeling
  "range_rarity",
  "population_density"
)
...
# wallacean geolocation model with global + realm-level slopes
# - time: years from species description to first geolocation (or censoring)
# - event: 1 = geolocation, 0 = not yet georeferenced by cutoff_year
...
priors <- c(
  ...
  prior(student_t(3, 0, 1), class = "sd")      # SDs of realm/country/family random effects
)
...
common_args <- list(
  ...
  threads   = threading(11, static = TRUE),  # Within-chain parallelization
  ...
  init      = 0,                          # Small positive initial values
...
for (config in model_configs) {
  ...
  raw_path   <- file.path(output_dir, sprintf("country_wallacean_%s_raw.rds",  model_name))
  final_path <- file.path(output_dir, sprintf("country_wallacean_%s.rds",      model_name))
...
if (length(fits) > 0L) {
  all_path <- file.path(output_dir, "country_wallacean_all.rds")
  ...
  warning("[WARNING] No models were successfully fitted; country_wallacean_all.rds not written.")
}
...
if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)
output_path <- file.path(output_dir, "country_wallacean_model_comparison")
...
saveRDS(fit_gomp, file.path(output_dir, "country_wallacean_gompertz.rds"))


# stan_survdata <- readRDS("output/stan_survdata_basin_wallacean_multiple.rds")
# fit_gomp <- brm_multiple(
#   formula = form_gompertz,
#   data    = stan_survdata,
#   chains  = 8,
#   cores   = 8,
