...
bi_legend <- ggplot() +
  ...
  scale_fill_identity() +
  geom_text(data = count_df,aes(x = x, y = y, label = percent(n_cells_ratio, accuracy = 0.1)),
...
dt <- inland %>% left_join(df_nsc[,c(1,10)], by = "iso3") %>%
  st_wrap_dateline(options = c("WRAPDATELINE=YES"))


p3 <- ggplot(data = dt) +
  #ggrastr::rasterise(geom_sf(data = world_map, fill = "lightgrey", colour = NA),dpi = 300)+
  ggrastr::rasterise(geom_sf(aes(fill = NSCI),colour = "white", linewidth = 0.03),dpi = 300)+
...
hist_df <- data.frame(
  xmin_count = 0,
  count = h_NSci$counts,
  ymin = head(h_NSci$breaks, -1),
  ymax = tail(h_NSci$breaks, -1),
  ymid = h_NSci$mids
) %>%
  mutate(
    width = count / max(count, na.rm = TRUE) * 0.22
  )

p3_legend <- ggplot(hist_df) +
  geom_rect(
    aes(xmin = -width,xmax = 0,ymin = ymin,ymax = ymax,fill = ymid),
    colour = NA,show.legend = F
  ) +
  scale_y_continuous(position = "right",expand = c(0,0),breaks = c(0,0.5,1))+
...
p33 <- ggdraw() +
  ...
  draw_plot(p3_legend, 0.12, 0.18, 0.26, 0.52)
...
ggdraw() +
  ...
  draw_label("B",x = 0.17, y = 1/3 + 0.33 - 0.02,
             fontface = "bold",size = 12,hjust = 0,vjust = 1) +
  draw_plot(p33, x = 0,   y = 0,   width = 1.0, height = 0.33)+
