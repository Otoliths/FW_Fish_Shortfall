...
p_bi_legend <- ggplot() +
  ...
  geom_text(
    data = count_df,
    aes(x = x, y = y, label = scales::percent(pct, accuracy = 0.1)),
...
p_nsc_fix <- ggdraw() +
  draw_plot(p_nsc, 0, 0, 1, 1) +
  draw_plot(legend, 0.12, 0.18, 0.26, 0.52)
...
p_final <- ggdraw() +
  ...
  draw_plot(p_nsc_fix, x = 0, y = 0, width = 1.0, height = 0.33) +
