...
# ---- Scenario table (q = 0.25) ----
df_q25 <- df %>%
  mutate(
    LS_state = within_realm_state(., "LS", q = 0.25),
    WS_state = within_realm_state(., "WS", q = 0.25),
    DS_state = within_realm_state(., "DS", q = 0.25),
    scenario = scenario_from_states(LS_state, WS_state, DS_state)
  )

# If you already have basin_shortfalls_fix.csv, keep using it.
# Otherwise you can write it from df_q25:
# dir.create(here("output", "tables"), showWarnings = FALSE, recursive = TRUE)
# write.csv(df_q25, path_out_tbl, row.names = FALSE)
